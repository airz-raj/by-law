...
| 2026-09-23 | Output token caps raised from 4 096 to 16 384 | The first real run decoded the cheque sample with 9/9 quotes confirmed, but the other two failed as `LLMOutputInvalid`. The newer Gemini models spend reasoning tokens from the same output budget, so a cap that only just fits the answer truncates it mid-JSON — which fails validation looking exactly like a schema mistake. Unused headroom costs nothing, because billing follows the tokens actually produced |
| 2026-09-23 | Truncation is told apart from a schema mistake | The two need different fixes. A cut-off answer is now asked to be shorter rather than corrected, and the error names the budget that ran out instead of a validation message about a field that was never wrong |
